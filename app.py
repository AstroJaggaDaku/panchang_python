...
KARANA = ["Kimstughna"] + ["Bava","Balava","Kaulava","Taitila","Garaja","Vanija","Vishti"]*8 + ["Shakuni","Chatushpada","Naga"]
...
# Classical Varjyam (degree inside nakshatra)
VARJYAM={
...
def jd(dt): return swe.julday(dt.year,dt.month,dt.day,dt.hour+dt.minute/60)
def from_jd(j,tz): return datetime.fromtimestamp((j-2440587.5)*86400,tz)
def calc(j,p): return swe.calc_ut(j,p,swe.FLG_SIDEREAL|swe.FLG_SWIEPH)[0][0]%360
def ang(a,b): return (a-b)%360

# =========================================================
# SUNRISE / SUNSET (AstroSage anchor)
# =========================================================
...
def panchang(date,lat,lon,tzname):
    ...
    kar="Kimstughna" if (t==0 and diff<6) else KARANA[int(diff/6)%60]
    ...
    for i in range(12):
        hora.append({"lord":HORA_SEQ[(idx+i)%7],
                     "from":(tcur+span*i).strftime("%H:%M"),
                     "to":(tcur+span*(i+1)).strftime("%H:%M")})
