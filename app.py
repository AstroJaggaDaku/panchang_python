...
def muhurta(sr,mins,seg):
    return sr+timedelta(minutes=seg*mins/8), sr+timedelta(minutes=(seg+1)*mins/8)

def panchang_for(date,lat,lon,tz):
    ...
    guli_s,guli_e=muhurta(sr,daymins,GULI_SEG[wd])
...
def api():
    ...
    return jsonify(panchang_for(dt,lat,lon,tz))
